...
ub = mean(prsdata.dQdCAfilt([2:2:NC],:))+2/sqrt(NC)*std(prsdata.dQdCA([2:2:NC],:));
lb = mean(prsdata.dQdCAfilt([2:2:NC],:))-2/sqrt(NC)*std(prsdata.dQdCA([2:2:NC],:));
...
ylabel('AHRR [J/deg]')
try
set(gca,'xlim',[prsdata.ca(1,prsdata.sse_index(1)) prsdata.mean_EOCfilt+20],'ylim',[-50 500]);
catch
...
try set(gca,'xlim',[prsdata.mean_SOCfilt-3 prsdata.mean_SOCfilt+2],'ylim',[min(mean_dQdCAfilt) 50],'fontsize',10,'position',[0.557018 0.75569 0.0909649 0.117871]);
...
for jj=1:length(prsdata.AHRRmax)
    cmAHRRavg_std(jj) = 1/sqrt(jj)*std(prsdata.AHRRmax(1:jj));
    cmAHRRavg_stdfilt(jj) = 1/sqrt(jj)*std(prsdata.AHRRmaxfilt(1:jj));
end
