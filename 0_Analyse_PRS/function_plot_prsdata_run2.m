...
catch set(gca,'xlim',[-10 10],'ylim',[min(mean_dQdCAfilt) max(mean_dQdCAfilt)],'fontsize',10);
end
...
ca2 = [prsdata.ca(1,:) fliplr(prsdata.ca(1,:))];
ub = mean(prsdata.dQdCAfilt([2:2:NC],:))+2/sqrt(NC/2-1)*std(prsdata.dQdCA([2:2:NC],:));
lb = mean(prsdata.dQdCAfilt([2:2:NC],:))-2/sqrt(NC/2-1)*std(prsdata.dQdCA([2:2:NC],:));
fill(ca2,[ub fliplr(lb)],[1 1 1]*.5,'FaceAlpha',0.5)
