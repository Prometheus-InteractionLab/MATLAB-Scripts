function [ fig,data ] = function_plot_processed_data_AHRRvsgIMEP(fig,mark ,colors,plotlegend,legendtext,digits,units)
...
for ii=1:length(filename)
    load(filename{ii})
    try data.ca = rundata.ca(1,:); catch data.ca = NaN; end
    ...
    try data.dQdCA(ii) = rundata.dQdCAavg(2,:); catch data.dQdCA(ii) = NaN; end
    try data.mean_gIMEP(ii) = rundata.mean_gIMEP; catch data.mean_gIMEP(ii) = NaN; end
...
p1 = subplot(2,3,1); hold on; grid on;
  h = errorbar(data.mean_gIMEP,data.mean_AHRRmax,data.std_AHRRmax,data.std_AHRRmax,mark);
...
p5 = subplot(2,3,2); hold on; grid on;
    h2 = errorbar(data.ca,data.dQdCA,)
