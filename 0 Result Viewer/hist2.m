function [h2d, binC_x, binC_y, mesh_minmaxN_xy] = ...
...
%h2d = zeros(N_x, N_y, 'single');
    ...
    %remove zero indices (added by Steve because zero indices are otherwise
    %unhandled; their physical representation is also not clear)
    t1=BinIx_xy(tmp_BinIx~=0,1);
    t2=BinIx_xy(tmp_BinIx~=0,2);
    BinIx_xy=[t1,t2];
    nP=numel(t1);
    t1=BinIx_xy(BinIx_xy(:,1)~=0,1);
    t2=BinIx_xy(BinIx_xy(:,1)~=0,2);
    BinIx_xy=[t1,t2];
    nP=numel(t1);
